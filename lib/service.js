...
var configFile = Path.join(config.util.getInitParam('NODE_CONFIG_DIR'), 'flickr_notification.json');
var serviceConfig = config.util.parseFile(configFile);
...
var service = function(){
    var self = this;
    this.sign_options = this.accounts_options = {};
    this.flickrs = {};

    async.series([
        function (next) {
            self.pg_connect(function (client, done) {
                var q = client.query('SELECT options FROM accounts', [privacy_id]);
                q.on('row', function (row) {
                    var id = row['account_id'];
                    self.accounts_options[id] = row['options'];
...
service.prototype.pg_connect = function (next) {
    pg.connect(this.options.connectionString, function (err, client, done) {
...
service.prototype.init = function() {
    ...
    async.forEach(Object.keys(this.accounts_options), function (key, callback) {
        Flickr.authenticate(self.accounts_options[key].api, function(err, flickr) {
            ...
            console.log(err, account + " auth success")
        });
        callback();
    }, function () {
...
service.prototype.checkPhotos = function(account_id, privacy_filter, topic){
    ...
    this.flickrs[account_id].people.getPhotos({
        api_key: self.options.api.api_key,
        user_id: self.options.api.user_id,
        per_page: 1,
        page: 1,
        privacy_filter: privacy_filter,

        oauth_consumer_key: self.options.api.api_key,
        oauth_token: self.options.api.access_token
    }, function(err, res){
        if (err) {
            console.error(err);
            throw err;
        ...
        // sendNotification();
        self.pg_connect(function (client, done) {
            var q = client.query('SELECT last_id FROM topics WHERE privacy_id = $1 and account_id = $2', [privacy_filter, account_id]);
            q.on('row', function (row) {
                if (flickrLastId != row['last_id']) {
                    q = client.query('UPDATE topics SET last_id = $1 WHERE privacy_id = $2 and account_id = $3',
                    ...
                    q.on('end', function () {
                        ...
                        self.sendNotification(res, topic);
                    });
                }
            });
            q.on('end', function () {
...
service.prototype.notify = function(login, next){
    ...
    if (login == null) {
        self.pg_connect(function (client, done) {
            var q = client.query('SELECT account_id, privacy_id, topic FROM topics');
            q.on('row', function (row) {
                self.checkPhotos(row['account_id'], row['privacy_id'], row['topic']);
                console.log('checkPhotos', row['account_id'], row['privacy_id'], row['topic'])
            }).on('end', function () {
                done();
                next();
            })
